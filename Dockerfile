...
RUN apt-get update; apt-get upgrade -y;DEBIAN_FRONTEND=noninteractive apt-get install -y locales
...
RUN sed -ie 's/\;date\.timezone\ \=/date\.timezone\ \=\ Europe\/Moscow/g' /etc/php5/cli/php.ini
RUN sed -ie 's/\;date\.timezone\ \=/date\.timezone\ \=\ Europe\/Moscow/g' /etc/php5/apache2/php.ini
